...
def literature_sources_path(workspace: RunWorkspace) -> Path:
    return workspace.literature_dir / "sources.json"


def literature_claims_path(workspace: RunWorkspace) -> Path:
    return workspace.literature_dir / "claims.json"


def validate_literature_evidence(workspace: RunWorkspace) -> list[str]:
    problems: list[str] = []
    sources_payload = _load_json_payload(literature_sources_path(workspace), "sources.json", problems)
    claims_payload = _load_json_payload(literature_claims_path(workspace), "claims.json", problems)
    ...
    for index, entry in enumerate(claims, start=1):
        if not isinstance(entry, dict):
            ...
            continue
        if not _clean_str(entry.get("claim_id")):
        ...
        if unknown_ids:
            problems.append(f"claims.json entry {index} references unknown source_ids: {', '.join(unknown_ids)}.")
    return problems
...
def validate_citation_verification(path: Path) -> list[str]:
    ...
    for index, entry in enumerate(claim_coverage, start=1):
        ...
        if not _clean_str(entry.get("claim")):
            problems.append(f"citation_verification.json claim_coverage entry {index} is missing a non-empty claim.")
        ...
        if not citation_keys and not source_ids:
            problems.append(
                f"citation_verification.json claim_coverage entry {index} must include citation_keys or source_ids."
            )
    return problems
